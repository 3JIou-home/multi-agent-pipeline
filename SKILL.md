...
Inspect run status, print the localized review summary, compile stage prompts with absolute references, copy prompts to the clipboard, launch `codex exec` for a chosen stage, run pending solvers in parallel, and surface `next: rerun` when verification says the goal is still incomplete.
